...
class SpamDB:
    ...
    def find_personal_reviews(self, reviewer_id: int):
        ...
        result = self.SQL_cursor.execute(
            '''
            SELECT Reviews.id, Phones.phone, Classes.name, Reviews.comment 
    ...
    def update_review(self, review_info: tuple):
        ...
        :param review_info: tuple (review_id, new_class_id, new_comment)
        :return: 1 - success, (-1) - error
        """
        ...
        self.SQL_cursor.execute(
            '''
            UPDATE Reviews
            SET class_id = ?, comment = ?
            WHERE id = ?
            ''', (review_info[1], review_info[2], review_info[0])
        ).fetchall()
